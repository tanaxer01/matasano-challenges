from collections import Counter
from os import walk
from typing import Tuple

CHAR_FREQ = {
    'a': 0.0651738, 'b': 0.0124248, 'c': 0.0217339, 'd': 0.0349835, 'e': 0.1041442, 'f': 0.0197881, 'g': 0.0158610,
    'h': 0.0492888, 'i': 0.0558094, 'j': 0.0009033, 'k': 0.0050529, 'l': 0.0331490, 'm': 0.0202124, 'n': 0.0564513,
    'o': 0.0596302, 'p': 0.0137645, 'q': 0.0008606, 'r': 0.0497563, 's': 0.0515760, 't': 0.0729357, 'u': 0.0225134,
    'v': 0.0082903, 'w': 0.0171272, 'x': 0.0013692, 'y': 0.0145984, 'z': 0.0007836, ' ': 0.1918182
}

def english_score(text: str) -> float:
    score = 0

    for letter, freq in Counter(text).items():
        if letter.lower() in CHAR_FREQ:
            score += CHAR_FREQ[letter.lower()] * freq

    return score

def break_xor_cipher(ciphered: bytes) -> Tuple[str, int, float]:
    ascii_text_chars = list(range(97, 122)) + [32]

    best = None
    for i in range(256):
        try:
            pos = [ i^k for k in ciphered ]
            score = sum([ i in ascii_text_chars for i in pos ])

            if best == None or best[2] < score:
                best = bytes(pos).decode(), i, score
        except:
            pass

    assert best != None 
    return best

if __name__ == "__main__": 
    hex = "1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736"
    hex_bytes = bytes.fromhex(hex)

    text, _, _ = break_xor_cipher(hex_bytes)
    print("challenge 3:\n\t", text)

